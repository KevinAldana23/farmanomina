  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full bg-muted/20 print:bg-white">
        ...
        <div>
          <div className="h-16 flex items-center justify-between px-6 bg-slate-950 font-bold text-white text-lg border-b border-slate-800">
            <span className="flex items-center gap-2">
              <span className="text-xl">⚕️</span> FarmaNómina
            </span>
            <button
              onClick={() => setMobileMenuOpen(false)}
          ...
          <nav className="py-4 flex flex-col gap-1.5 px-4 overflow-y-auto max-h-[calc(100vh-170px)]">
            {NAV_ITEMS.map(item => {
      ...
      <aside className="hidden md:flex md:w-64 bg-slate-900 text-slate-300 flex-shrink-0 flex-col justify-between print:hidden min-h-screen">
        <div>
          <div className="h-16 flex items-center px-6 bg-slate-950 font-bold text-white text-lg border-b border-slate-800/80">
            ⚕️ FarmaNómina
          </div>
